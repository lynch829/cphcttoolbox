...
# --- BEGIN_HEADER ---
#
# genflux2proj - cone beam intensity to projection value converter
...
"""Generate projs from cone beam intensity value files"""

import sys

from cphct.cone.conf import str_value, default_cone_npy_conf, \
...
from cphct.cone.npycore.utils import inline_tool_helper, default_tool_opts, \
    ...
    engine_app_aliases, flux_to_proj, check_norm
...
def tool_opts():
    ...
    opts.update({
        ...
        'handler': str_value,
        'default': None,
        'description': 'Background intensity value or path to file with ' \
                'detector pixel matrix of values',
        },
        'air_norm': {
        'long': 'air-norm',
        'short': None,
        ...
        'handler': str_value,
        'default': None,
        'description': 'Pure air intensity value or path to file with ' \
                'detector pixel matrix of values',
        },
        })
    return opts
...
def core_tool(projs, conf, tool_state):
    ...
    zero_norm_matrix = load_helper_proj(conf['zero_norm'], conf, idt)
    air_norm_matrix = load_helper_proj(conf['air_norm'], conf, idt)
    check_norm(zero_norm_matrix, air_norm_matrix)
    return flux_to_proj(projs, zero_norm_matrix, air_norm_matrix,
                 (conf['detector_rows'], conf['detector_columns']))


def main(conf):
    """Generate projs from intensity values
