...
"""OpenCL specific kernel initialization helper functions"""
...
from cphct.cl.io import cl_alloc, get_cl_data
...
def __prepare_gpu_kernels(conf):
    ...
    prepared_kernels['flat_diff_chunk'] = cl_kernels.flat_diff_chunk
    prepared_kernels['flat_diff_chunk'].set_scalar_arg_dtypes([int32,
            int32, None, None])

    # flat_fwd_rebin_chunk

    prepared_kernels['flat_fwd_rebin_chunk'] = \
        cl_kernels.flat_fwd_rebin_chunk

    prepared_kernels['flat_fwd_rebin_chunk'
                     ].set_scalar_arg_dtypes([int32, int32, None, None])

    # flat_convolve_chunk

    prepared_kernels['flat_convolve_chunk'] = \
        cl_kernels.flat_convolve_chunk
    ...
    prepared_kernels['flat_rev_rebin_chunk'] = \
        cl_kernels.flat_rev_rebin_chunk
    ...
    prepared_kernels['flat_backproject_chunk'] = \
        cl_kernels.flat_backproject_chunk
    prepared_kernels['flat_backproject_chunk'].set_scalar_arg_dtypes([
        int32,
        int32,
        int32,
        int32,
    ...
    prepared_kernels['curved_diff_chunk'] = cl_kernels.curved_diff_chunk
    prepared_kernels['curved_diff_chunk'].set_scalar_arg_dtypes([int32,
            int32, None, None])

    # curved_fwd_rebin_chunk

    prepared_kernels['curved_fwd_rebin_chunk'] = \
        cl_kernels.curved_fwd_rebin_chunk
    ...
    prepared_kernels['curved_convolve_chunk'] = \
        cl_kernels.curved_convolve_chunk
    ...
    prepared_kernels['curved_rev_rebin_chunk'] = \
        cl_kernels.curved_rev_rebin_chunk
    ...
    prepared_kernels['curved_backproject_chunk'] = \
        cl_kernels.curved_backproject_chunk
    prepared_kernels['curved_backproject_chunk'].set_scalar_arg_dtypes([
        int32,
        int32,
        int32,
        int32,
    ...
    prepared_kernels['checksum_array'] = cl_kernels.checksum_array
    prepared_kernels['checksum_array'].set_scalar_arg_dtypes([None,
            None, int32, int32])
...
def init_recon(conf, fdt):
    """
    Initialize data structures for OpenCL Katsevich reconstruction
    ...
    -------
    output : dict
       Returns configuration dictionary filled with OpenCL Katsevich
       data structures
    """
    ...
    if conf['proj_filter'] != 'skip':
        cl_mirrors.append('proj_filter_array')

    if conf['checksum']:
        cl_mirrors.append('check_dest')

    total_size = 0
    for gpu_array_name in cl_mirrors:
        npy_array = get_npy_data(conf, gpu_array_name)
        gpu_array = gpuarray.zeros(gpu_queue, npy_array.shape,
                                   npy_array.dtype)

        cl_alloc(conf, gpu_array_name, gpu_array, gpu_array.nbytes)
        logging.debug('cl allocing %dB for %s' % (gpu_array.nbytes,
                      gpu_array_name))

        total_size += gpu_array.nbytes
        logging.debug('cl alloc size for %s is %s - total %s'
